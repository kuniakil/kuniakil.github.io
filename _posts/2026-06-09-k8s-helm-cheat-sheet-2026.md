...
> [!NOTE]
> 服務跑起來之後，Helm 的任務就暫時結束了，底層運作依然是標準的 K8s 元件。
...
當您在使用 Helm 時，除了基本的安裝和升級外，管理**軟體商店（Repos）**和**設定變數（Values）**是日常最頻繁的操作。

### 1. 軟體商店（Repository）管理
* **新增商店**：
  ```bash
  helm repo add <商店名稱> <商店網址>
  ```
  *(類比：新增 App Store 的第三方商店源。例如：`helm repo add jetstack https://charts.jetstack.io`)*
* **列出已新增的商店**：
  ```bash
  helm repo list
  ```
* **更新商店目錄（同步最新版本資訊）**：
  ...
  helm repo update
  ```
  *(類比：`apt update` 或 `brew update`。不會變動執行中的服務，僅更新本機的軟體目錄)*
* **在商店中搜尋軟體**：
  ```bash
  helm search repo <關鍵字>
  ```
  *(例如：`helm search repo wordpress`。會搜尋您所有已安裝商店中的相關軟體)*

### 2. 設定參數（Values）管理
* **查看原廠預設的 `values.yaml`（查閱可用參數）**：
  ```bash
  helm show values <商店名稱>/<軟體名稱>
  ```
* **將原廠預設設定導出成實體檔案（供修改用）**：
  ```bash
  helm show values <商店名稱>/<軟體名稱> > my-values.yaml
  ```
* **查詢目前叢集中「正在執行」的客製化設定**：
  ```bash
  helm get values <Release名稱> -n <命名空間>
  ```
  *(非常實用！當您忘記當初安裝時下了什麼參數，用此指令可一秒從 K8s 讀回)*